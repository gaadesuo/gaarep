...
grid_0 = [0, 0]
grid_1 = [0, 0]
for chake_word in inp_word:
    # 前回間違えぎりぎりのフラグが立ってない場合
    if grid_0[0] == 0 and grid_0[1] == 0:
        if chake_word in side_limit:
            # 間違えぎりぎりをタッチしたので文字からx, y座標を求める
            if chake_word in all_word[0]:
                grid_0 = [all_word[0].find(chake_word), 0]
            elif chake_word in all_word[1]:
                grid_0 = [all_word[0].find(chake_word), 1]
            else:
                grid_0 = [all_word[0].find(chake_word), 2]
    ...
    # 前回間違えのフラグが立ってる場合
        ...
        # 今回の座標取得
        if chake_word in all_word[0]:
            grid_1 = [all_word[0].find(chake_word), 0]
        elif chake_word in all_word[1]:
            grid_1 = [all_word[0].find(chake_word), 1]
        else:
            grid_1 = [all_word[0].find(chake_word), 2]
        ...
        if -1 <= diff_grid[0] <= 1 or -1 <= diff_grid[1] <= 1:
            counter += 1
            grid_0 = grid_1
        else:
            grid_0 = [0, 0]
